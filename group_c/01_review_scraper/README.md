...
- [scrape_reviews.py](scrape_reviews.py): The core script that fetches HTML (from a URL or local file) and extracts review data.
- [config.json](config.json): A configuration file containing CSS selectors for specific page elements (reviewer name, rating, body, etc.).
- [data/sample_product_page.html](data/sample_product_page.html): A mock e-commerce page for demonstration and safe testing.
- [outputs/](outputs/): Directory where the extracted data is stored in `reviews.json` and `reviews.csv`.

### Key Functions:
- `load_html()`: Handles fetching content from a URL or reading a local file.
- `extract_jsonld_reviews()`: ([line 38](scrape_reviews.py#L38)) Attempts to find `application/ld+json` scripts which often contain structured review data (Schema.org).
- `extract_with_selectors()`: ([line 82](scrape_reviews.py#L82)) Fallback method using CSS selectors from the config file.
